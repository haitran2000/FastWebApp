using ATSCADAWebApp.Core;
using System;
using System.ComponentModel;
using System.Text;
using System.Windows.Forms;
using System.Xml.Serialization;

namespace ATSCADAWebApp.Component.AlarmReporter
{
    /// <summary>
    /// Bao cao alarm
    /// </summary>
    [DisplayName("iAlarmReporter")]
    [Serializable()]
    [XmlType(TypeName = "atscada-alarm-reporter")]
    public class ATSCADAAlarmReporter : ComponentBase, ISupportGrid, ISupportRender
    {
        #region FIELDS

        private string content = "Alarm Reporter";

        private string connection = "atscada";

        private string tableName = "alarmlog";

        private decimal timeout = 30000;

        private string gridColumn = "col-sm-12";

        #endregion

        #region PROPERTIES
        /// <summary>
        /// Tieu de hien thi cua report tren UI
        /// </summary>

        [XmlAttribute("at-content")]
        public string Content
        {
            get => this.content;
            set => SetField(ref this.content, value);
        }

        /// <summary>
        /// Chuoi ket noi toi DATABASE de kiem tra dang nhap, login
        /// Chuoi connection duoc cai dat trong file webConfig
        /// Gom: Server, Port, User, Pass, Database
        /// </summary>
        [XmlAttribute("at-connection")]
        public string Connection
        {
            get => this.connection;
            set => SetField(ref this.connection, value);
        }

        /// <summary>
        /// Table alarm log
        /// </summary>
        [XmlAttribute("at-table-name")]
        public string TableName
        {
            get => this.tableName;
            set => SetField(ref this.tableName, value);
        }

        /// <summary>
        /// Thoi gian timeout request alarm log recocrd
        /// </summary>
        [XmlAttribute("at-timeout")]
        public decimal Timeout
        {
            get => this.timeout;
            set => SetField(ref this.timeout, value);
        }

        /// <summary>
        /// So cot hien thi trong Column. Theo bootsrap grid
        /// </summary>
        [XmlAttribute("at-grid-column")]
        public string GridColumn
        {
            get => this.gridColumn;
            set => SetField(ref this.gridColumn, value);
        }

        #endregion

        #region CONSTRUCTORS

        public ATSCADAAlarmReporter()
            : base()
        {
            Name = "NewAlarmReporter";
            Description = "AlarmReporter - ATSCADA Web Component";
        }

        public ATSCADAAlarmReporter(IComposite parent)
            : base(parent)
        {
            Name = "NewAlarmReporter";
            Description = "AlarmReporter - ATSCADA Web Component";
        }

        public ATSCADAAlarmReporter(string name, IComposite parent)
            : base(name, parent)
        {
            Description = "AlarmReporter - ATSCADA Web Component";
        }

        #endregion

        #region METHODS

        /// <summary>
        /// UI cap nha thuoc tinh
        /// </summary>
        /// <returns></returns>
        public override bool Update()
        {
            return new ATSCADAAlarmReporterEditor(this).ShowDialog() == DialogResult.OK;
        }

        /// <summary>
        /// Render thanh CustomHTMLElement
        /// atscada-alarm-reporter
        /// </summary>
        /// <returns></returns>
        public string Render()
        {
            var builder = new StringBuilder();

            builder.Append($"<div class=\"{GridColumn}\"><atscada-alarm-reporter ");
            builder.Append($"at-content=\"{Content}\" ");
            builder.Append($"at-connection=\"{Connection}\" ");
            builder.Append($"at-table-name=\"{TableName}\" ");
            builder.Append($"at-timeout=\"{Timeout}\" >");
            builder.AppendLine($"</atscada-alarm-reporter></div>");

            return builder.ToString();
        }

        #endregion
    }
}
